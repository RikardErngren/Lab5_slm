...
public:
    TextGenerator(const LanguageModel& slm, const int text_length) : slm(slm) {}
    std::string generate_text() {
        int output_length = 0;

        // Generating text specifications
        // 5.
        while (output_length < text_length)
        {
            // 1.
            std::string k_gram = choose_starting_k_gram();
            // 2.
    ...
    std::string choose_starting_k_gram() {
        ...
        for (const auto& [k_gram, freq] : slm.k_gram_freqs) {
        ...
        return prev(slm.k_gram_freqs.end())->first; // prev(slm.end()) gives last key-value pair, ->first gives key
    }
    ...
    char sample_next_character(std::string k_gram) {
        ...
        float cumulative_freq = 0.0f;
        for (const auto& [next_char, freq] : slm.transition_freqs[k_gram]) {
            if (random_number >= cumulative_freq && random_number <= freq) {
                return next_char;
            }
            cumulative_freq += freq;
        }
        return prev(slm.transition_freqs[k_gram].end())->first;
    }
