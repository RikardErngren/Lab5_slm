...
std::string TextGenerator::choose_k_gram() {
    ...
    return std::prev(slm.k_gram_freqs.end())->first; // prev(slm.end()) gives last key-value pair, ->first gives key
}
...
char TextGenerator::sample_next_character(std::string k_gram) {
    ...
    return std::prev(slm.transition_freqs[k_gram].end())->first;
}